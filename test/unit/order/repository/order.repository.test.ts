...
describe('OrderRepository Unit Test', () => {
  ...
  it ('should create an order with one provider product', async () => {
    ...
    const oneProduct: ProductOrderDto[] = products.slice(0,1).map(product => ({
      productId: product.productId,
      quantity: 1
    }));
    const orderWithAddress: OrderWithAddressDto = new OrderWithAddressDto(1,1,client.address, oneProduct);
  ...
  it ('should create an order with all provider products', async () => {
    ...
    const orderWithAddress: OrderWithAddressDto = new OrderWithAddressDto(1,1,client.address, productOrderDto);
    const order = await orderRepository.createOrder(orderWithAddress);
  ...
  it ('should not create an order with invalid providerId', async () => {
    ...
    const orderWithAddress: OrderWithAddressDto = new OrderWithAddressDto(1,99999,client.address, productOrderDto);
    const order = await orderRepository.createOrder(orderWithAddress);
