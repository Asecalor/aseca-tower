...
import {
  ...
  getClientAddressMockWithSuccesfulResponse, getClientAddressMockWithUnSuccesfulResponse,
...
describe ('Order Service Unit Test', () => {
  ...
  it ('should create an order with one provider product',async ()=>{
    ...
    orderRepositoryMock.createOrder.mockImplementation(
      (order) => {
  ...
  it ('should not create an order with not existing product', async () => {
    ...
    await expect(orderService.createOrder(createOrderDto)).rejects.toThrow(
      new ConflictException('Some of products where not found or it not belongs to the provider')
    );
  })
})